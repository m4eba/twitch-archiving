import { FileConfigOpt, PostgresConfigOpt, } from '@twitch-archiving/config';
...
import { FileStatus, } from '@twitch-archiving/prisma/prisma/generated/rec-client/index.js';
...
const logger = initLogger('missing-db-files');
...
pool.end();
async function findMissing(recording, total, missing) {
    const result = Array.from({ length: missing.length }, (x, i) => -1);
    console.log('find', recording.site_id.substring(5));
    const recordings = await pool.query('SELECT * FROM recording WHERE streamid = $1', [recording.site_id.substring(5)]);
    const ids = recordings.rows.map((r) => r.id);
    for (let i = 0; i < missing.length; ++i) {
        const found = await pool.query('SELECT * FROM file WHERE seq = $1 AND recording_id = ANY($2::int[])', [missing[i], ids]);
