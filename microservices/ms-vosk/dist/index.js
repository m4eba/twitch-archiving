...
async function getSession(recordingId, tmpOutput) {
...
function createWebSocket(recordingId) {
...
async function prepareSegment(session, tmpOutput) {
    if (session.msg === null) {
...
async function sendSegment(session, filename, tmpOutput) {
    return new Promise(async (resolve, reject) => {
        ...
        while (true) {
            ...
            if (bytesRead < session.byteOffset)
                continue;
...
async function responseMessage(session) {
    ...
    if (session.firstMsg === null || session.msg === null) {
        logger.error({ msg: session.msg, fistMsg: session.firstMsg }, 'responseMessage null value');
